"""
EPSTEIN FORENSIC WORKBOOK v6.1
================================
Major update: Demonstrates HOW the money moved, not just WHAT moved.
$1,964,229,742 (104.6% of $1.878B SAR benchmark)

Changes from v6:
  - NEW TAB: "Money Flow Patterns" — shows EXTERNAL→SHELL→SHELL→EXTERNAL circuit
  - NEW TAB: "Shell Trust Hierarchy" — 4-tier trust network with in/out per entity
  - FIXED: Entity classifier now handles underscore names (SOUTHERN_TRUST, HAZE_TRUST)
  - FIXED: Shell Network now correctly identifies 34 shell-to-shell wires
  - UPDATED: All column names rewritten for public clarity
  - UPDATED: Entity P&L rebuilt with correct flow types
  - UPDATED: Master Ledger columns renamed for storytelling

Tabs (11):
  1. Executive Summary       — headline numbers, three-tier framework
  2. Extraction Phases       — 24-phase pipeline, running totals
  3. Money Flow Patterns     — ★ NEW: how money moved through the network
  4. Shell Trust Hierarchy   — ★ NEW: 4-tier trust structure with flows
  5. Master Wire Ledger      — 382 wires, every transaction we found
  6. Above-Cap Verified      — 8 wires that broke the $10M ceiling
...
OUTFILE = "EPSTEIN_FORENSIC_WORKBOOK_v6.xlsx"
...
def load_master_ledger():
    for p in LEDGER_PATHS:
        if p.exists():
            with open(p) as f:
                data = json.load(f)
            print(f"  Loaded: {p} ({len(data)} entries)")
            return data
    print("  master_wire_ledger_phase24.json not found")
    return []
...
def main():
    print("=" * 70)
    print("  BUILDING FORENSIC WORKBOOK v6.1")
    print("  $1,964,229,742 — 104.6% SAR Coverage")
    print("=" * 70)
    print()
    ledger = load_master_ledger()
    if not ledger:
        print("\n  Cannot build without master ledger. Exiting."); sys.exit(1)
    print(f"  Ledger: {len(ledger)} wires, ${sum(w['amount'] for w in ledger):,.0f}")
    ...
    print(f"\n  Saved: {local_path}")
    if HDD_DIR.exists():
        import shutil; shutil.copy2(local_path, HDD_DIR / OUTFILE)
        print(f"  Copied to HDD: {HDD_DIR / OUTFILE}")

    print(f"\n{'=' * 70}")
    print(f"  FORENSIC WORKBOOK v6.1 COMPLETE — 11 TABS")
    print(f"  Total: ${GRAND_TOTAL:,.0f} ({GRAND_TOTAL/SAR_TOTAL*100:.1f}% SAR)")
    print(f"{'=' * 70}")
